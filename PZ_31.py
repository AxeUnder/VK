...
Перед вам класс ContextDictionary, в котором уже реализованы несколько методов:
    ...
    put(self, key, value), который кладет значение value по ключу key в self.dictionary, когда этот атрибут является
    словарем, то есть в контексте with;
    get(self, key), который забирает значение по ключу key в self.dictionary, когда этот атрибут является словарем, то
    есть в контексте with.
